...
async def create_match_via_api(base_url: str, *, real: bool = False,
    ...
    """通过后端 API 自动创建一局 6 座位 p6-classic，返回对局 ID。
...
async def run_tui_mode(match_id: int, port: int, *, real: bool = False) -> None:
    ...
    if match_id == 0:
        ...
        match_id = await create_match_via_api(base_url, real=real)
