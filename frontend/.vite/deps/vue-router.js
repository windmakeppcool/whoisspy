...
// node_modules/vue-router/dist/devtools-CN5uWJaH.js
...
function tokenizePath(path) {
  ...
  if (!isAbsolutePath(path)) throw new Error(true ? `Route paths should start with a "/": "${path}" should be "/${path}".` : `Invalid path "${path}"`);
...
function createRouter(options) {
  ...
  function addRoute(parentOrRoute, route) {
    ...
    const removeRoute2 = matcher.addRoute(record, parent);
    routesVersion.value++;
    return () => {
      removeRoute2();
      routesVersion.value++;
  ...
  function finalizeNavigation(toLocation, from, isPush, replace2, data) {
    ...
    if (isPush) {
      if (replace2 || isFirstNavigation) routerHistory.replace(toLocation.fullPath, assign({ scroll: isFirstNavigation && state && state.scroll }, data));
...
vue-router/dist/devtools-CN5uWJaH.js:
